...
// - Apre: mezzanotte del giorno dell'evento (00:00)
// - Chiude: endTime del giorno dell'evento (se < 12:00 = giorno dopo); senza endTime = giorno+1 alle 12:00
...
export async function POST(req: NextRequest) {
  ...
  // Verifica che l'evento appartenga al club e controlla la finestra temporale
  if (ticket.event_id) {
    ...
    if (!isWithinEventWindow(event.date, event.end_time)) {
      ...
      const isPast = new Date() > closeAt;
      return NextResponse.json({
        ok: false,
        reason: isPast ? 'event_ended' : 'not_yet',
  ...
  if (ticket.status === 'used') {
    return NextResponse.json({
      ok: false,
      reason: 'already_used',
      usedAt: new Date(ticket.created_at).toLocaleTimeString('it-IT', {
        hour: '2-digit',
        minute: '2-digit',
      }),
    });
